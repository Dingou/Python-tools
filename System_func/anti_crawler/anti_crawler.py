...
nginx_reload_cmd = "ls -al"

def ngx_time_interval(line_num):
    try:
        ngx_headlog_cmd = tail_raw_cmd % (line_num, ngx_log, grep_head_partten)
        (status, stdout) = commands.getstatusoutput(ngx_headlog_cmd)
...
class Anti_Crawler(object):
    ...
    #TODO Firstly add the config to /etc/hosts.deny, then conf.d/blockips.conf
